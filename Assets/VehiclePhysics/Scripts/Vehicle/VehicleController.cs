...
namespace NWH.VehiclePhysics
{
    ...
    public partial class VehicleController : MonoBehaviour
    {
        ...
        [HideInInspector] public InputStates input = new InputStates();//用于存储车辆的输入状态。
        ...
        /// <summary>
        /// 如果有车轮打滑True if any of the wheels is skidding (slipping in the lateral / side direction)
        /// </summary>
        [ShowInTelemetry]
        ...
        /// <summary>
        ///如果超过2s没有任何活动(移动或用户输入) Returns true if there has been no activity - either movement or user input - for over 2s.
        /// </summary>
        ...
        private void Awake()
        {
            ...
            transmission.SetTransmissionType(VehicleInputMgr.GetInstance().transmType);
        }
        ...
        public static float GetMouseHorizontal()
        {
            ...
            float percent = Mathf.Clamp(Mouse.current.position.ReadValue().x / Screen.width, -1f, 1f);
            if (percent < 0.5f)
        ...
        {
            float percent = Mathf.Clamp(Mouse.current.position.ReadValue().y / Screen.height, -1f, 1f);
            if (percent < 0.5f)
                return -(0.5f - percent) * 2.0f;
            return (percent - 0.5f) * 2.0f;
        }
