    ...
    public class Tut_5_CustomObjectLocalPivot : MonoBehaviour
    {
        ...
        private void Update()
        {
                ...
                RTGizmosEngine.Get.HoveredGizmo == null)
            {
                ...
                if (pickedObject.gameObject.layer != LayerMask.NameToLayer("Entity") && pickedObject.gameObject.layer != LayerMask.NameToLayer("Obstacle"))
                {
            ...
            if (Keyboard.current.gKey.isPressed) SetTransformSpace(GizmoSpace.Global);
            else if (Keyboard.current.lKey.isPressed) SetTransformSpace(GizmoSpace.Local);
            ...
            if (Keyboard.current.fKey.isPressed) SetWorkGizmoId(GizmoId.Move);
            else if (Keyboard.current.rKey.isPressed) SetWorkGizmoId(GizmoId.Rotate);
            else if (Keyboard.current.cKey.isPressed) { _selectedObjects.Clear();OnSelectionChanged(); }
        ...
        private void OnSelectionChanged()
        {
            ...
            if (_selectedObjects.Count != 0)
            {
                ...
                {
                    if (_selectedObjects[i].GetComponent<EntityBase>())
                    {
                        _selectedObjects[i].GetComponent<EntityBase>().isTransformGizmo = true;
                    }
                }
            }
            else
            {
                ...
                for (int i = 0; i < _selectedObjects.Count; i++)
                {
                    if (_selectedObjects[i].GetComponent<EntityBase>())
                    {
                        _selectedObjects[i].GetComponent<EntityBase>().isTransformGizmo = false;
                    }
                }
            }
        }
        ...
        public void ChoiceObj(GameObject pickedObject)
        {
            if (pickedObject.GetComponent<Terrain>() != null) return;
            // The CTRL key is not pressed; in this case we just clear the selection and
            // select only the object that we clicked on.
            _selectedObjects.Clear();
            _selectedObjects.Add(pickedObject);
