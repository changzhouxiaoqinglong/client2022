...
public class TerrainModifierPanel : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            TerrainChange.TerrainModule.Undo();
        }
    }
    ...
    public void OnLeftButtonHold()
    {
        ...
        switch (ModifierType)
        {
            case ModifierType.Terrain:
                switch (TerrainModifierType)
                {
                    case TerrainModifierType.Up:
                        TerrainChange.TerrainModule.ChangeHeightWithBrush(hitInfo.point, slider1.Value, slider2.Value, prototypeIndex, true, true);
                        break;
                    case TerrainModifierType.Down:
                        TerrainChange.TerrainModule.ChangeHeightWithBrush(hitInfo.point, slider1.Value, slider2.Value, prototypeIndex, false, true);
                        //Debug.Log("hitinfo" + hitInfo);
                        //Debug.Log("hitinfo.point" + hitInfo.point);
                        //Debug.Log("slier1" + slider1.Value);
                        //Debug.Log("slier2" + slider2.Value);
                        break;
                    case TerrainModifierType.Smooth:
                        TerrainChange.TerrainModule.Smooth(hitInfo.point, slider1.Value, slider2.Value, regesterUndo:true);
                        break;
                    default:
                        break;
                }
            ...
            case ModifierType.Detial:
                if (isAdd)
                {
                    TerrainChange.TerrainModule.AddDetial(hitInfo.point, slider1.Value, (int)slider2.Value, prototypeIndex, true);
                    Debug.Log("isadd");
                }
                else
                {
                    TerrainChange.TerrainModule.RemoveDetial(hitInfo.point, slider1.Value, prototypeIndex, true);
                    Debug.Log("not is add");
                }
                break;
            case ModifierType.Texture:
                TerrainChange.TerrainModule.SetTexture(hitInfo.point, slider1.Value, prototypeIndex, 0.05f * slider2.Value, true);
                break;
            default:
                break;
        }
    ...
    private void OnLeftButtonUp()
    {
        ...
        if (hitInfo.collider != null) 
        {
            switch (ModifierType)
            {
                case ModifierType.Tree:
                    if (isAdd)
                        TerrainChange.TerrainModule.CreatTree(hitInfo.point, (int)slider2.Value, (int)slider1.Value, prototypeIndex);
                    else
                        TerrainChange.TerrainModule.RemoveTree(hitInfo.point, (int)slider1.Value, prototypeIndex);
                    break;
                default:
                    break;
            }
