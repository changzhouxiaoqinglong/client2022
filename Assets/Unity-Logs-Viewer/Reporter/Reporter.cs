...
public class Reporter : MonoBehaviour
{
	...
	Vector2 getDownPos()
	{
		if (Application.platform == RuntimePlatform.Android ||
		   Application.platform == RuntimePlatform.IPhonePlayer) {
            ...
            //if (Input.GetMouseButtonDown(0)) { 
            if (Mouse.current.leftButton.wasPressedThisFrame) {
                downPos.x = Mouse.current.position.ReadValue().x;//Input.mousePosition.x;
				downPos.y = Mouse.current.position.ReadValue().y;//Input.mousePosition.y;
                return downPos;
			}
		}
	...
	Vector2 getDrag()
	{
		...
		if (Application.platform == RuntimePlatform.Android ||
			...
			//	return Vector2.zero;
			//}
            return Vector2.zero; // Input.touches[0].position - downPos;
		}
		else {
			//if (Input.GetMouseButton(0)) {
            if (Mouse.current.leftButton.isPressed)
            {
                mousePosition = Mouse.current.position.ReadValue();//Input.mousePosition;
