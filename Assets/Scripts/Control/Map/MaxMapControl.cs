    ...
    private void InitEndPoint()
    {
        ...
        if (SceneMgr.GetInstance().curScene is Train3DSceneCtrBase scene3D)
        {
            ...
            foreach (var harmArea in scene3D.harmAreaMgr.areaList)
            {
                ...
                float windSp = taskEnvVarData.Wearth.GetWindSp();
                harmObj.transform.GetComponent<RectTransform>().localScale = new Vector3(range, range/ harmArea.GetCurvePosition(windSp), 1);
