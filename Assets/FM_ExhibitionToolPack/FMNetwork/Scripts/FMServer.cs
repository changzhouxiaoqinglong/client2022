...
public class FMServer : MonoBehaviour
{
    ...
    public class ConnectedClient
    {
        ...
        public void Send(byte[] _byte)
        {
            ...
            {
                Logger.LogError("clients: " + _byte.Length + " " + e);
                Close();
            }
        }
        public void Close()
    ...
    public void Action_CheckClientStatus(string _ip)
    {
        ...
        if (!IsExistedClient)
        {
            ...
            Logger.Log(TAG + "Add FM Client : " + _ip);
    ...
    void Sender()
    {
        for (int i = ConnectedClients.Count - 1; i >= 0; i--)
        {
            ...
            try
            {
                ...
                {
                    //remove it if didn't receive any data from client for 3000 ms
                    Logger.LogDebug(TAG + "Remove FM Client : " + ConnectedClients[i].IP);
                    ConnectedClients[i].Close();
                    ConnectedClients.Remove(ConnectedClients[i]);
                }
            }
            ...
            {
                //这里加了try catch
                //之前偶尔这里的逻辑会有报错，导致Sender方法的循环调用中断 画面无法同步，报错原因未知
                //这里加上try catch 看是否能解决问题
                Logger.LogError(TAG + "SenderError: " + e.ToString());
            }
        }
