...
/// <summary>
/// 同步画面插件
/// 需要注意的几个值  ServerListenPort服务器监听端口号  ClientListenPort客户端监听端口号
/// 发送画面（Server） 和 接受画面(Client) 两端这两个端口号要一样
/// Client.ServerIP  客户端同步画面对应的服务端IP，可以指定同步某个服务端的画面(比如同步驾驶位置的画面，就要填写驾驶位的IP)，前提是勾掉AutoNetWorkDiscovery，
/// 如果不勾掉，他就会在整个局域网内自动寻找可以对应匹配的Server端
/// GameViewEncoder 和 Decode两个脚本里的label值，是分组，发送和接收方必须label一样，才能正常接收解析画面，目前每辆车是同一个label值。
/// </summary>
public class FMNetworkManager : MonoBehaviour
{
    ...
    void Init()
    {
        if (NetworkType == FMNetworkType.Server)
        {
            ...
            Server.ServerListenPort = ServerListenPort;
            Server.ClientListenPort = ClientListenPort;
        ...
        else
        {
            ...
            Client.ServerListenPort = ServerListenPort;
            Client.ClientListenPort = ClientListenPort;
            ...
            if (!Client.AutoNetworkDiscovery) Client.ServerIP = NetConfig.SYS_SCREEN_IP;
    ...
    void Update()
    {
        ...
        #region Debug Status
        string _status = "";
        _status += "Thread: " + FmLoom.numThreads + " / " + FmLoom.maxThreads + "\n";
